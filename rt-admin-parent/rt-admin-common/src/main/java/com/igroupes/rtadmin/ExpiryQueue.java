...
import com.igroupes.rtadmin.util.TimeUtils;
...
public class ExpiryQueue<E> {
    ...
    public ExpiryQueue(int expirationInterval) {
        ...
        nextExpirationTime.set(roundToNextInterval(TimeUtils.currentElapsedTime()));
    }
    ...
    public Long update(E elem, int timeout) {
        ...
        long now = TimeUtils.currentElapsedTime();
        // 重新计算新的过期时间
    ...
    public void dump(PrintWriter pwriter) {
        ...
        for (long time : keys) {
            ...
            if (set != null) {
                ...
                pwriter.print(TimeUtils.elapsedTimeToDate(time));
                pwriter.println(":");
                for (E elem : set) {
                    pwriter.print("\t");
